...
class SandboxResult:
    def __init__(self, success: bool, stdout: str = "", stderr: str = "", exit_code: int = 0, execution_time_ms: int = 0, malware_detected: bool = False, malware_details: list = None):
...
class DevSandbox:
    ...
    async def execute(self, code: str, test_code: str = "") -> "SandboxResult":
        ...
        if detected:
            return SandboxResult(success=False, stderr="Code rejected by malware scanner", exit_code=1, malware_detected=True, malware_details=details)
        with tempfile.TemporaryDirectory(prefix="sandbox_") as tmpdir:
            ...
            try:
                ...
                try:
                    ...
                    proc.kill()
                    return SandboxResult(success=False, stdout="", stderr="Execution timed out", exit_code=-1)
